...
INTERF_BATCHES ?= 2000
interf-eval: install
	uv run python -m tests.interference.interfere_eval --batches $(INTERF_BATCHES) --nproc $(NPROC)

interf-test: install
	uv run python -m tests.interference.interfere_eval --no-interference --batches 100 --nproc $(NPROC)
